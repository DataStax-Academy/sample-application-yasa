...
schema.vertexLabel('cluster').partitionKey('cluster_id').properties('cluster_id', 'cluster_size', 'confidence_level','golden_company_name',  'golden_display_name',  'golden_company_reg_no','golden_customer_name', 'golden_customer_type', 'golden_dob', 'golden_firstname', 'golden_surname').ifNotExists().create()
schema.vertexLabel('customer').partitionKey('src_customer_id').properties('src_customer_id', 'cd_si_ext', 'customer_type', 'firstname',  'surname',  'company_reg_no','company_name', 'customer_type', 'dob').ifNotExists().create()
schema.vertexLabel('contract').partitionKey('agreement_id').properties('agreement_id', 'src_customer_id', 'sys_apl','agreement_start_date',  'maturity_date',  'terminated_date','contract_type', 'vin_no', 'status_id').ifNotExists().create()
schema.vertexLabel('vehicule').partitionKey('vin_no').properties('vin_no').ifNotExists().create()
...
schema.vertexLabel('cluster').index('search').search().by('golden_display_name').asText().ifNotExists().add()
schema.vertexLabel('customer').index('search').search().by('firstname').asText().ifNotExists().add()
schema.vertexLabel('cluster').index('byGolden').materialized().by('golden_display_name').ifNotExists().add()
schema.vertexLabel('customer').index('bySurname').materialized().by('surname').ifNotExists().add()
schema.vertexLabel('customer').index('byfirstName').materialized().by('firstname').ifNotExists().add()